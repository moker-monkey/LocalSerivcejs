...
describe('GET method', () => {
    before(() => {
        LocalService.listener('http://localhost:8080/api/test/success', 'GET', (req, resolve, reject) => {
            resolve({
                status: 201,
                response: {msg: 'this message is resolve'}
            })
        })
        ...
        LocalService.listener('http://localhost:8080/api/test/error', 'GET', (req, resolve, reject) => {
            reject({
                status: 401,
                response:{message: 'this message is not'}
            })
        })
    })
    it('success', (down) => {
